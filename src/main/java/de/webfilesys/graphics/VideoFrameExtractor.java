...
public class VideoFrameExtractor extends Thread
{
    ...
    public void run() {
        if (LogManager.getLogger(getClass()).isDebugEnabled()) {
            LogManager.getLogger(getClass()).debug("starting video frame extractor thread for video file " + videoFilePath);
        }
        ...
        if (!CommonUtils.isEmpty(ffmpegExePath)) {
            ...
            if (!frameDirFile.exists()) {
                if (!frameDirFile.mkdir()) {
                    LogManager.getLogger(getClass()).error("failed to create video frame directory " + videoFramePath);
                }
            }
			...
			try {
				...
				LogManager.getLogger(getClass()).error("failed to grab frame from video " + videoFilePath, ioex);
			} catch (InterruptedException iex) {
				LogManager.getLogger(getClass()).error("failed to grab frame from video " + videoFilePath, iex);
			}
        }
    }
