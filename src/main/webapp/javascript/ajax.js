function cutCopyToClip(fileName, operation) {
    ...
    xmlGetRequest("cutCopy", parameters, (responseXml) => handleCopyResult(responseXml));
}

function handleCopyResult(xmlDoc) {
    const item = xmlDoc.getElementsByTagName("message")[0];
    ...
    if (xmlDoc.getElementsByTagName("copyOperation").length > 0) {
        addCopyAllowed = true;
    ...
    if (xmlDoc.getElementsByTagName("moveOperation").length > 0) {
        addMoveAllowed = true;
    ...
    if (xmlDoc.getElementsByTagName("enablePaste").length > 0) {
        if (document.getElementById("pasteButton")) {
            document.getElementById("pasteButton").style.display = "inline";
        }
    }
    if (xmlDoc.getElementsByTagName("enablePasteAsLink").length > 0) {
...
function editLocalLink(filePath) {
    const parameters = { "filePath": encodeURIComponent(filePath) };
    xmlGetRequest("editFile", parameters, () => hideMenu());
}
...
function diffSelect(path) {
    ...
    xmlGetRequest("diffSelect", parameters, responseXml => {
...
function cancelDiff() {
    ...
    xmlGetRequest("diffSelect", parameters,
        responseXml => {
            parent.diffStarted = false;
            hideMenu();
            stopMenuClose = true;
...
function checkPasteOverwrite(path) {
    ...
    const parameters = !!path ? { "path": encodeURIComponent(path) } : {};

    xmlGetRequest("checkPasteOverwrite", parameters, responseXml => {

        let pasteUrl = "/webfilesys/servlet?command=pasteFiles";

        var pathElems = responseXml.getElementsByTagName("path");
        if (pathElems && (pathElems.length == 1)) {
            const path = pathElems[0].firstChild.nodeValue;
            pasteUrl = pasteUrl + "&actpath=" + encodeURIComponent(path);
        }

        var destFolderFileConflictElem = responseXml.getElementsByTagName("destFolderFileConflict");
        if (destFolderFileConflictElem && (destFolderFileConflictElem.length > 0)) {
            customAlert(resourceBundle["destFolderFileConflict"]);
            return;
        }

        var targetEqualsSourceElem = responseXml.getElementsByTagName("targetEqualsSource");
        if (targetEqualsSourceElem && (targetEqualsSourceElem.length > 0)) {
            customAlert(resourceBundle["targetEqualsSource"]);
            return;
        }

        var targetIsSubOfSourceElem = responseXml.getElementsByTagName("targetIsSubOfSource");
        if (targetIsSubOfSourceElem && (targetIsSubOfSourceElem.length > 0)) {
            customAlert(resourceBundle["targetIsSubOfSource"]);
            return;
        }

        var conflicts = responseXml.getElementsByTagName("conflict");

        if (conflicts.length > 0) {
            let msg;
            const folderElem = responseXml.getElementsByTagName("folder");
